...
\ni Soit $(a,b,c)$ les sommets d'un triangle. Une premiere etape et de
...
\wedge(c - a)$.
\dd\ni On veut maintenant verifier que le point $p$ est dans le demi-plan 
contenant $c$ et etant delimite par la droite passant par $a$ et $b$ (note
$P_{a,b}$).
\begin{center}
...
\end{tikzpicture}
\end{center}
\dd\ni Il suffit de regarder le signe du produit scalaire entre deux vecteurs :
$$\vec{v}_1 = (b - a)\wedge(p - a)\etsp \vec{v}_2 = (b - a)\wedge(c - a)$$
\ni Les deux cas possibles sont representes sur la figure, $\vec{v}$ est
colineaire a la normale et oriente comme $\vec{n}$ ssi $p$ est dans le
demi-plan. Ainsi :
